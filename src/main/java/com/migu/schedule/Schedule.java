...
{
    // 任务挂起队列
    public List<TaskInfo> freeTaskList;
    ...
    public int init()
    {
        freeTaskList = new LinkedList<TaskInfo>();
    ...
    public int registerNode(int nodeId)
    {
        ...
        List<TaskInfo> taskInfosList = new LinkedList<TaskInfo>();
        // 将新节点注册，并初始化节点下的任务
        nodeAndTasks.put(nodeId, taskInfosList);
    ...
    public int unregisterNode(int nodeId)
    {
        ...
        List<TaskInfo> taskInfosList = nodeAndTasks.get(nodeId);
        // 如果该节点里有处理中的任务则把任务放到挂起列表中
        if (!isEmpty(taskInfosList))
        {
            freeTaskList.addAll(taskInfosList);
        }
        // 删除节点
    ...
    public int addTask(int taskId, int consumption)
    {
        ...
        for (TaskInfo taskinfo : freeTaskList)
        {
    ...
    public int deleteTask(int taskId)
    {
        ...
        int index = freeTaskList.indexOf(taskinfotemp);
        freeTaskList.remove(index);
    ...
    public int scheduleTask(int threshold)
    {
        ...
        if (!isEmpty(freeTaskList))
        {
            List<TaskInfo> freeTaskListtemp = new LinkedList<TaskInfo>();
            freeTaskListtemp.addAll(freeTaskList);
            for (TaskInfo taskInfo : freeTaskListtemp)
            {
